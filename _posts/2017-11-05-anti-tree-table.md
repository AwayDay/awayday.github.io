...
사실 구글에서 검색하면 더 쉽게 정리된 내용이 나오기야 할 것이다(...)
...
이 정도의, 조금 더 깊은 생각이 필요한 특징 말이다. 물론 더 __엄밀하게__ 따지면, Object와 명세의 그 어느 사이에 낀 _인터페이스_ 나 _추상 클래스_ 가 지금 하고자 하는 이야기를 복잡하게-인터페이스는 다중 구현이 가능하다.- 만들겠지만, 내가 이 비유를 바탕으로 글을 쓰고 싶어하기 때문에-억지라는건 잘 알고 있다.- 과감하게 생략하였다.
...
```sql
SELECT *
FROM obj_table AS ot
WHERE '1/2/5/6/' LIKE ot.path || '%';
/* path = 경로 */
```
